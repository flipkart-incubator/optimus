"""
Classes defining basic NN building blocks and layers
...
class MLPDropout(object):
    ...
    def __init__(self,rng=None,input=None,layer_sizes=None,dropout_rates=None,activations=None,use_bias=True,layers=None):
        if(layers!=None):         #From pickle file
            self.layers = layers
    ...
    def __setstate__(self, state):
         self.layers,self.activations = state
...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2), non_linear="tanh"):
        ...
        if self.non_linear=="none" or self.non_linear=="relu":
            rng_uniform = rng.uniform(low=-0.01, high=0.01, size=filter_shape)
            self.W = theano.shared(numpy.asarray(rng_uniform,dtype=theano.config.floatX),borrow=True,name="W_conv")
        else:
    ...
    def __getstate__(self):
        return (self.b,self.W,self.image_shape,self.filter_shape,self.non_linear,self.poolsize)
    def __setstate__(self, state):
         self.b,self.W,self.image_shape,self.filter_shape,self.non_linear,self.poolsize = state
